...
项目使用 Node.js 18+ 开发，数据保存在本地 SQLite 文件中。系统采用单 Bot、单后台管理员和单 Telegram 管理员模式，不包含会员、余额、充值账户或多商户功能。所有商品统一以 USDT 定价，每笔订单独立结算。

## 功能概览

### Telegram 客户端

- `/start` 启动商店，展示选购、历史订单、联系客服和开源项目入口。
- 按分类浏览商品，商品介绍支持 Telegram Markdown。
- 下单时依次填写数量和商品扩展参数。
- 扩展参数支持选择项和文本输入项，可作为 API 发货变量使用。
- 支持 TRC20-USDT、OKPay 和通用易支付三种付款方式。
- 历史订单每页显示 10 条，可查看付款状态、处理状态和发货结果。
- 已付款但自动处理失败的订单可以申请自动重试。
- 客服模式支持文本、图片和文件，发送 `/cancel` 退出客服会话。
- 系统不向客户展示 API 原始响应、HTTP 状态或上游接口错误，只显示成功或失败结果。

Telegram 管理员账号只接收系统通知，不在 Bot 内处理商品和订单。管理员发送 `/start` 时，Bot 会提示前往 Web 后台管理。
...
- 管理员可以回复文本、图片和文件。
- 管理员回复会在 Telegram 中加上“来自管理员的回复”提示。
...
- 支持明暗主题切换，主题选择保存在当前浏览器中。
- 右上角可以修改管理员密码，修改时必须验证原密码。
...
系统仅识别 TRON 主网官方 USDT 合约：

```text
TR7NHqjeKQxGTCi8q8ZY4pL8otSzgjLj6t
```

每笔订单会在商品金额上增加 `0.001` 至 `0.099 USDT` 的随机尾数。未过期 TRC20 订单的应付金额全局唯一，用于在共用地址或多个通道之间准确匹配付款。
...
OKPay 通道需要填写 App ID、密钥和 API 地址。默认 API 地址为：

```text
https://api.okaypay.me/shop/
```
...
易支付是通用协议，系统不绑定默认服务商。商品 USDT 价格会按照后台设置的 USDT/CNY 汇率换算为人民币，并保留两位小数。

系统同时接收异步通知，并每 5 秒主动查询未过期订单。实际接口路径和签名规则需与所使用的易支付服务商兼容。
...
1. 打开“系统设置”。
2. 填写 Telegram Bot API Token。
3. 填写接收通知的 Telegram 管理员 ID。
4. 设置 Bot 名称、订单超时时间和 USDT/CNY 汇率。
5. 如果使用支付回调，填写公网 HTTPS 地址。
6. 保存设置并确认后台显示 Bot 已连接。
7. 创建商品分类。
8. 创建商品并配置库存或发货 API。
9. 添加至少一个支付通道。
10. 使用普通 Telegram 账号向 Bot 发送 `/start`，完成一次小额测试订单。
...
发布前请把默认 Telegram 管理员 ID 修改为自己的账号 ID。
...
cd /opt/tg-shop-bot
sudo systemctl stop shop-bot
sudo tar -czf /var/backups/tg-shop-bot-before-update-$(date +%F-%H%M).tar.gz data
sudo -u shopbot git pull --ff-only
sudo -u shopbot npm ci --omit=dev
sudo systemctl start shop-bot
sudo journalctl -u shop-bot -n 100 --no-pager
```

不要删除或覆盖 `data` 目录。程序启动时会自动检查数据库结构。
...
常见浏览器的强制刷新快捷键为 `Ctrl+F5`。如果前面有 CDN 或代理缓存，也需要清理对应缓存。
...
运行后生成的 `data` 和 `login.txt` 已在 `.gitignore` 中排除。
